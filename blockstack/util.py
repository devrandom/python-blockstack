...
class _UnsetTimeoutKlass(object):
    """ A sentinel for an unset timeout. Defaults to the system timeout. """
    def __repr__(self):
        return '<Unset Timeout Value>'


# None has special meaning for timeouts, so we use this sentinel to indicate
# that we don't care
UNSET_TIMEOUT = _UnsetTimeoutKlass()


class BlockstackException(Exception):
    pass
